...
python3 GCPPEAS.py --resource agent-auth-provider:projects/victim-project/locations/us-central1/authProviders/known-provider --only-specified
python3 GCPPEAS.py --resource integration-connector:projects/victim-project/locations/us-central1/connections/known-connection --only-specified
```
...
3. **Live read-only probes:** If both methods above are incomplete, AWSPEAS tests only read command families (`List`, `Get`, `Describe`, `BatchGet`, `Head`, `Lookup`, and `Search`). The AWS CLI is optional unless this fallback is needed. Help parsing supports plain, groff, and Unicode bullet formats, with botocore service models as an OS-independent fallback. For WorkMail and the WorkSpaces family, AWSPEAS can also recover option-matched resource IDs from successful CloudTrail history before using safe dummy IDs; failed-event parameters are rejected to prevent false-positive feedback loops. Use `--aws-services` to limit requests or `--bruteforce-always` to check for resource-policy access even after complete IAM reads.
